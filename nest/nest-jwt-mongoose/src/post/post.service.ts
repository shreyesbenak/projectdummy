...
export class PostService {
  constructor(
    @Inject(POST_MODEL) private postModel: Model<BlogPost>,
    @Inject(COMMENT_MODEL) private commentModel: Model<Comment>,
    @Inject(REQUEST) private req: AuthenticatedRequest,
  ) { }

  findAll(keyword?: string, skip = 0, limit = 10): Observable<BlogPost[]> {
    if (keyword) {
      return from(
        this.postModel
  ...
  save(data: CreatePostDto): Observable<BlogPost> {
    ...
    const createPost: Promise<BlogPost> = this.postModel.create({
      ...data,
